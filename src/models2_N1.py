...
class PatchWiseModel2(BaseModel2):
    ...
    def train(self):
        self.network.train()
        print('Start training patch-wise network (RESNET 448 x 336): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
...
class ImageWiseModel2(BaseModel2):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel2, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights2N1_' + image_wise_network.name() + '.pth')
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('N1 Context-aware ResNet Scale II')
