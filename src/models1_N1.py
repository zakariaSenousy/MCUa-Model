...
class PatchWiseModel1(BaseModel1):
    def __init__(self, args, network):
        super(PatchWiseModel1, self).__init__(args, network, args.checkpoints_path + '/weights1_' + network.name() + '.pth')

    def train(self):
        self.network.train()
        print('Start training patch-wise network (DENSENET 296 x 224): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))

        train_loader = DataLoader(
            dataset=PatchWiseDataset1(path=self.args.dataset_path + TRAIN_PATH, stride=self.args.patch_stride1, rotate=True, flip=True, enhance=True),
    ...
    def validate(self, verbose=True):
        ...
        test_loader = DataLoader(
            dataset=PatchWiseDataset1(path=self.args.dataset_path + VALIDATION_PATH, stride=self.args.patch_stride1),
            batch_size=self.args.batch_size,
            shuffle=False,
            num_workers=4
        )
    ...
    def test(self, path, verbose=True):
        self.network.eval()
        dataset = TestDataset1(path=path, stride=self.args.patch_stride1, augment=False)
        data_loader = DataLoader(dataset=dataset, batch_size=1, shuffle=False)
        stime = datetime.datetime.now()

        if verbose:
            print('Finalising Second Model DenseNet Scale III')
    ...
    def visualize(self, path, channel=0):
        self.network.eval()
        dataset = TestDataset1(path=path, stride=PATCH_SIZE)
        data_loader = DataLoader(dataset=dataset, batch_size=1, shuffle=False)
...
class ImageWiseModel1(BaseModel1):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel1, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights1N1_' + image_wise_network.name() + '.pth')

        self.patch_wise_model = PatchWiseModel1(args, patch_wise_network)
    ...
    def train(self):
        ...
        print('Start training image-wise network (N1): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        dataset = TestDataset1(path=path, stride=self.args.patch_stride1, augment=ensemble)
        data_loader = DataLoader(dataset=dataset, batch_size=1, shuffle=False)
        stime = datetime.datetime.now()

        if verbose:
            print('N1 Context-aware DenseNet Scale III')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                patches = patches.view(n_bins, -1, 336, 322)
            ...
            for i in range(n_bins):
                ...
                newPatch = np.concatenate((np.array(patches[i][0]), np.array(patches[i][1])))
                newPatch1 = np.concatenate((np.array(patches[i][1]), np.array(patches[i][2])))
                newPatch2 = np.concatenate((np.array(patches[i][2]), np.array(patches[i][3])))
                newPatch3 = np.concatenate((np.array(patches[i][3]), np.array(patches[i][4])))
                newPatch4 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][5])))
                newPatch5 = np.concatenate((np.array(patches[i][5]), np.array(patches[i][6])))
                newPatch6 = np.concatenate((np.array(patches[i][6]), np.array(patches[i][7])))
                newPatch7 = np.concatenate((np.array(patches[i][7]), np.array(patches[i][8])))
                ...
                Single_test_image.append(newPatch4)
                Single_test_image.append(newPatch5)
                Single_test_image.append(newPatch6)
                Single_test_image.append(newPatch7)
...
"""
    ...
    def _patch_loader(self, path, augment):
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            dataset = ImageWiseDataset1(
                path=path,
                stride=self.args.patch_stride1,
                flip=augment,
                rotate=augment,
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                res = res.view((bsize, -1, 336, 322)).data.cpu().numpy()
                ...
                for i in range(bsize):
                    ...
                    newPatch = np.concatenate((np.array(res[i][0]), np.array(res[i][1])))
                    newPatch1 = np.concatenate((np.array(res[i][1]), np.array(res[i][2])))
                    newPatch2 = np.concatenate((np.array(res[i][2]), np.array(res[i][3])))
                    newPatch3 = np.concatenate((np.array(res[i][3]), np.array(res[i][4])))
                    newPatch4 = np.concatenate((np.array(res[i][4]), np.array(res[i][5])))
                    newPatch5 = np.concatenate((np.array(res[i][5]), np.array(res[i][6])))
                    newPatch6 = np.concatenate((np.array(res[i][6]), np.array(res[i][7])))
                    newPatch7 = np.concatenate((np.array(res[i][7]), np.array(res[i][8])))
                    
                    single_Image.append(newPatch)
                    single_Image.append(newPatch1)
                    single_Image.append(newPatch2)
                    single_Image.append(newPatch3)
                    single_Image.append(newPatch4)
                    single_Image.append(newPatch5)
                    single_Image.append(newPatch6)
                    single_Image.append(newPatch7)
