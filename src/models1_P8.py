...
class ImageWiseModel1(BaseModel1):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel1, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights1P8_' + image_wise_network.name() + '.pth')
    ...
    def train(self):
        ...
        print('Start training image-wise network (P8): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P8 Context-aware DenseNet Scale III')
