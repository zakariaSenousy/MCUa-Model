...
class ImageWiseModel1(BaseModel1):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel1, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights1P5_' + image_wise_network.name() + '.pth')
    ...
    def train(self):
        ...
        print('Start training image-wise network (P5): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P5 Context-aware DenseNet Scale III')
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                for i in range(bsize):
                    ...
                    newPatch2 = np.concatenate((np.array(res[i][4]), np.array(res[i][5]),
