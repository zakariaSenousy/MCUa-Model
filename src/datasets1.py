...
class PatchWiseDataset1(Dataset):
    def __init__(self, path, stride=PATCH_SIZE, rotate=False, flip=False, enhance=False):
        super().__init__()

        wp = int((296 - PATCH_SIZE) / stride + 1)
        hp = int((224 - PATCH_SIZE) / stride + 1)
    ...
    def __getitem__(self, index):
        ...
        with Image.open(self.names[im]) as img:
            ...
            img1 = img.resize((296, 224)) #scale III
...
class ImageWiseDataset1(Dataset):
    def __init__(self, path, stride=PATCH_SIZE, rotate=False, flip=False, enhance=False):
        super().__init__()
