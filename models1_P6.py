...
class ImageWiseModel1(BaseModel1):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel1, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights1P6_' + image_wise_network.name() + '.pth')
    ...
    def train(self):
        ...
        print('Start training image-wise network (P6): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P6 Context-aware DenseNet Scale III')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            for i in range(n_bins):
                ...
                newPatch1 = np.concatenate((np.array(patches[i][3]), np.array(patches[i][4]),
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                for i in range(bsize):
                    ...
                    newPatch1 = np.concatenate((np.array(res[i][3]), np.array(res[i][4]),
