...
class ImageWiseModel1(BaseModel1):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel1, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights1P4_' + image_wise_network.name() + '.pth')
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P4 Context-aware DenseNet Scale III')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            for i in range(n_bins):
                ...
                newPatch = np.concatenate((np.array(patches[i][0]), np.array(patches[i][1]),
                                               np.array(patches[i][2]), np.array(patches[i][3])))
                    
                newPatch1 = np.concatenate((np.array(patches[i][2]), np.array(patches[i][3]),
                                               np.array(patches[i][4]), np.array(patches[i][5])))
                    
                newPatch2 = np.concatenate((np.array(patches[i][1]), np.array(patches[i][2]),
                                               np.array(patches[i][3]), np.array(patches[i][4])))
                    
                newPatch3 = np.concatenate((np.array(patches[i][3]), np.array(patches[i][4]),
                                               np.array(patches[i][5]), np.array(patches[i][6])))
                
                newPatch4 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][5]),
                                               np.array(patches[i][6]), np.array(patches[i][7])))
                
                newPatch5 = np.concatenate((np.array(patches[i][5]), np.array(patches[i][6]),
                                               np.array(patches[i][7]), np.array(patches[i][8])))
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                for i in range(bsize):
                    ...
                    newPatch = np.concatenate((np.array(res[i][0]), np.array(res[i][1]),
                                               np.array(res[i][2]), np.array(res[i][3])))
                    
                    newPatch1 = np.concatenate((np.array(res[i][2]), np.array(res[i][3]),
                                               np.array(res[i][4]), np.array(res[i][5])))
                    
                    newPatch2 = np.concatenate((np.array(res[i][1]), np.array(res[i][2]),
                                               np.array(res[i][3]), np.array(res[i][4])))
                    
                    newPatch3 = np.concatenate((np.array(res[i][3]), np.array(res[i][4]),
                                               np.array(res[i][5]), np.array(res[i][6])))
                
                    newPatch4 = np.concatenate((np.array(res[i][4]), np.array(res[i][5]),
                                               np.array(res[i][6]), np.array(res[i][7])))
                
                    newPatch5 = np.concatenate((np.array(res[i][5]), np.array(res[i][6]),
                                               np.array(res[i][7]), np.array(res[i][8])))
